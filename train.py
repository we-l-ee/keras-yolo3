...
def _main():
    ...
    save_best_only = True if val_split !=0 else False

    logging = TensorBoard()
    checkpoint = ModelCheckpoint(os.path.join(log_dir,'ep{epoch:05d}-loss{loss:.3f}'+val_print+'.h5'),
        monitor='val_loss', save_weights_only=True, save_best_only=save_best_only, period=config.getint("Train","backup_period"))
    ...
    if config.getboolean("Train", "Stage1"):
        ...
        model.fit_generator(data_generator_wrapper(lines[:num_train], batch_size, input_shape, anchors, num_classes),
                ...
                epochs=config.getint("Train","stage1_epoch"),
                initial_epoch=0,
    ...
    if True:
        ...
        model.fit_generator(data_generator_wrapper(lines[:num_train], batch_size, input_shape, anchors, num_classes),
            ...
            epochs=config.getint("Train", "stage2_epoch"),
            initial_epoch=config.getint("Train","last_epoch"),
            callbacks=[checkpoint,reduce_lr], verbose=verbose)
